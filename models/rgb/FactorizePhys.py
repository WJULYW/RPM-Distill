...
class _MatrixDecompositionBase(nn.Module):
    ...
    def local_inference(self, x, bases):
        # Ensure bases and x are on the same device
        bases = bases.to(x.device)
    ...
    def forward(self, x, return_bases=False):
        ...
        if not self.rand_init and not hasattr(self, 'bases'):
            ...
            # Note: bases will be moved to x.device when used in forward
            self.register_buffer('bases', bases)

        # (S, D, R) -> (B * S, D, R)
        if self.rand_init:
            bases = self._build_bases(B, self.S, D, self.R)
            bases = bases.to(x.device)  # Ensure bases are on the same device as x
        else:
            bases = self.bases.repeat(B, 1, 1).to(x.device)
...
class BVP_Head(nn.Module):
    def __init__(self, md_config, device, frames, dropout_rate=0.1, debug=False):
        ...
        if self.use_fsam:
            ...
            self.bias1 = nn.Parameter(torch.tensor(1.0, device=device), requires_grad=True)
        else:
            inC = nf[2]
        ...
        self.final_layer_hr = nn.Sequential(
            ConvBlock3D(inC, nf[1], [3, 3, 3], [1, 1, 1], [1, 0, 0]),  # B, nf[1], 160, 5, 5
            ConvBlock3D(nf[1], nf[0], [3, 3, 3], [1, 1, 1], [1, 0, 0]),  # B, nf[0], 160, 3, 3
            nn.Conv3d(nf[0], 1, (frames-1, 3, 3), stride=(frames-1, 3, 3), padding=(0, 0, 0), bias=False)   # B, 1, 1, 1, 1,
        )
    ...
    def forward(self, voxel_embeddings, batch, length):
        ...
        if (self.md_infer or self.training or self.debug) and self.use_fsam:
            ...
            if self.md_res:
                bias1 = self.bias1.to(voxel_embeddings.device)
                x = torch.mul(voxel_embeddings - voxel_embeddings.min() + bias1,
                              att_mask - att_mask.min() + bias1)
            ...
            hr = self.final_layer_hr(factorized_embeddings)  # includes spatial dimensions

            # Reduce singleton and spatial dimensions to get one HR value per sample.
            hr = hr.squeeze()  # e.g. [3,1,1,13,13] -> [3,13,13]
            hr = torch.mean(hr, dim=(-2, -1))  # average over H and W -> [batch]
            hr = hr.view(batch, 1)  # reshape to [batch, 1]

        else:
            x = self.final_layer(voxel_embeddings)

        # Keep rPPG handling explicit and stable.
        x = x.squeeze(1)  # remove channel dimension -> [batch, T, H, W]
        x = torch.mean(x, dim=(-2, -1))  # average over spatial dimensions -> [batch, T]
        rPPG = x.view(batch, length)  # [batch, length]
        ...
        if (self.md_infer or self.training or self.debug) and self.use_fsam:
            return rPPG, hr, factorized_embeddings, appx_error
        else:
            return rPPG, hr


class FactorizePhys(nn.Module):
    def __init__(self, frames=model_config["frames"], md_config=model_config, in_channels=3, dropout=0.1, device=torch.device("cuda" if torch.cuda.is_available() else "cpu"), debug=False):
        ...
        self.rppg_head = BVP_Head(md_config, device=device, frames=frames, dropout_rate=dropout, debug=debug)
    ...
    def forward(self, x):  # [batch, Features=3, Temp=frames, Width=32, Height=32]
        ...
        if (self.md_infer or self.training or self.debug) and self.use_fsam:
            rPPG, hr, factorized_embeddings, appx_error = self.rppg_head(voxel_embeddings, batch, length - 1)
        else:
            rPPG, hr = self.rppg_head(voxel_embeddings, batch, length - 1)
        rPPG = rPPG.view(batch, length - 1)
...
if __name__ == "__main__":
    model = FactorizePhys(frames=160, md_config=model_config, in_channels=3, dropout=0.1, device=torch.device("cpu"), debug=True)
    input = torch.randn(2, 3, 160, 72, 72)
    output = model(input)
    print("-" * 20)
    print(output[0].shape) # (batch, 159)
    print(output[1].shape) # (batch, 1)