...
def rotateIQ(iq_array):
    ...
    rotation_mat = np.array([[np.cos(theta), -np.sin(theta)], [np.sin(theta),np.cos(theta)]], dtype=np.float32)
    rotation_tensor = torch.tensor(rotation_mat, dtype=iq_array.dtype).to(iq_array.device)
    
    # Handle different tensor dimensions
    if iq_array.dim() == 3:  # (batch, channels, frames)
        # Reshape to (batch, 2, channels//2, frames) for rotation
        batch_size, channels, frames = iq_array.shape
        if channels % 2 == 0:
            iq_reshaped = iq_array.view(batch_size, 2, channels//2, frames)
            for i in range(channels//2):
                iq_reshaped[:, :, i, :] = torch.matmul(rotation_tensor, iq_reshaped[:, :, i, :])
            iq_array = iq_reshaped.view(batch_size, channels, frames)
    elif iq_array.dim() == 4:  # (batch, 2, channels, frames)
        for i in range(iq_array.shape[2]):
            iq_array[:,:,i,:] = torch.matmul(rotation_tensor, iq_array[:,:,i,:])
