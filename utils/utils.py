...
def extract_video(path: str, file_str: str, length: int = 900, 
    ...
    """
    if iio is None:
        raise ImportError("imageio is required by extract_video() but is not installed.")
    video = []
...
def pulse_rate_from_power_spectral_density(pleth_sig: np.array, FS: float = 30,
                                           LL_PR: float = 40, UL_PR: float = 200,
                                           BUTTER_ORDER: int = 6,
                                           DETREND: bool = False,
